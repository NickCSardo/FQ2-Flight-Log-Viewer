...
function addZone(zone, map, color){
    ...
    google.maps.event.addListener(zPolygon, 'mouseover', function(e) {
        infowindow.setContent('<b>'+ zone.Code +'</b><br />' + zone.LowerBound + ' - ' + zone.UpperBound + ' ft');
        infowindow.setPosition(coords[0]);
        infowindow.open(map);
    });

    google.maps.event.addListener(zPolygon, 'mouseout', function() {
        infowindow.close();
    });
}
...
function drawAttributeChart(divId, attributeName, maxY, flights, selector){
    ...
    var data = _.map(flights, function(f,i){
        ...
        var records = _.map(_.filter(f.path, function(p,i){return (i == f.path.length -1) || selector(p) > 0.0}), function(r,i){
            return {
